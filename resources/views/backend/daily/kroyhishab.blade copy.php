                            ...
                            <tbody>
                                @foreach($dailyKroy as $daily)
                                    <tr>
                                        ...
                                        <td data-paikar-id="{{ $daily->customer->id ?? '' }}">{{ $daily->customer->name ?? 'N/A' }}</td>
                                        ...
                                        <td>
                                            <form action="{{ route('daily.destroy', $daily->id) }}" method="POST" class="delete-form d-inline">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="totalKg" name="total_qty" class="form-control" required>
            </div>

            <div class="mb-3">
                <label class="form-label">প্রতি কেজির চার্জ (৳)</label>
                <input type="number" id="chargePerKg" name="charge_per_kg" class="form-control" placeholder="যেমন 5" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="date" id="date" name="date" class="form-control" value="{{ \Carbon\Carbon::now('Asia/Dhaka')->toDateString() }}">
            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#chargeModal').on('show.bs.modal', function () {
                ...
                let paikarName = $('.dataTables_filter input').val() || 'N/A';
                let totalKg = $('#totalQty').text();

                let firstRow = $('.datanew tbody tr:visible').first();
                let paikarId = firstRow.find('td:eq(3)').data('paikar-id') || '';
